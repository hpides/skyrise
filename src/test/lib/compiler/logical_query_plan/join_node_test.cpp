...
TEST_F(JoinNodeTest, FunctionalDependenciesSemiAndAntiJoins) {
  // Preparations
  const auto fd_a = FunctionalDependency{{t_a_a_}, {t_a_b_}};
  const auto fd_x = FunctionalDependency{{t_b_x_}, {t_b_y_}};
...
TEST_F(JoinNodeTest, FunctionalDependenciesForwardNonTrivialBothAndDerive) {
  for (const auto join_mode :
    ...
    const auto generated_fd_b_c = FunctionalDependency{{t_a_b_, t_a_c_}, {t_a_a_}};
    const auto generated_fd_y = FunctionalDependency{{t_b_y_}, {t_b_x_}};
...
TEST_F(JoinNodeTest, FunctionalDependenciesDeriveNone) {
  ...
  const auto fd_b = FunctionalDependency{{t_a_b_}, {t_a_a_}};
  const auto fd_y = FunctionalDependency{{t_b_y_}, {t_b_x_}};
  ...
  const auto generated_fd_a = FunctionalDependency{{t_a_a_}, {t_a_b_, t_a_c_, t_b_x_, t_b_y_}};
  EXPECT_EQ(join_node->FunctionalDependencies().at(2), generated_fd_a);
  const auto generated_fd_x = FunctionalDependency{{t_b_x_}, {t_b_y_, t_a_a_, t_a_b_, t_a_c_}};
...
TEST_F(JoinNodeTest, FunctionalDependenciesDeriveLeftOnly) {
  ...
  // Tests
  const auto generated_fd_a = FunctionalDependency{{t_a_a_}, {t_a_b_, t_a_c_}};
  EXPECT_EQ(join_node->NonTrivialFunctionalDependencies().size(), 1);
  EXPECT_EQ(join_node->NonTrivialFunctionalDependencies().at(0), generated_fd_a);

  const auto generated_fd_x = FunctionalDependency{{t_b_x_}, {t_a_a_, t_a_b_, t_a_c_, t_b_y_}};
...
TEST_F(JoinNodeTest, FunctionalDependenciesUnify) {
  ...
  const auto fd_a_b = FunctionalDependency{{t_a_a_, t_a_b_}, {t_a_c_}};
  ...
  const auto expected_fd_a_b = FunctionalDependency{{t_a_a_, t_a_b_}, {t_a_c_, t_b_x_, t_b_y_}};
  const auto expected_fd_c = FunctionalDependency{{t_a_c_}, {t_a_a_, t_a_b_, t_b_x_, t_b_y_}};
  const auto expected_fd_x = FunctionalDependency{{t_b_x_}, {t_a_a_, t_a_b_, t_a_c_, t_b_y_}};
...
TEST_F(JoinNodeTest, UniqueConstraintsInnerAndOuterJoins) {
  ...
  for (const auto join_mode : {JoinMode::kInner, JoinMode::kLeftOuter, JoinMode::kRightOuter, JoinMode::kFullOuter}) {
    ...
    // In-depth checks
    EXPECT_TRUE(find_unique_constraint_by_key_constraint(*key_constraint_a_, join_unique_constraints));
    EXPECT_TRUE(find_unique_constraint_by_key_constraint(*key_constraint_b_c_, join_unique_constraints));
    EXPECT_TRUE(find_unique_constraint_by_key_constraint(*key_constraint_x_, join_unique_constraints));
    EXPECT_TRUE(find_unique_constraint_by_key_constraint(*key_constraint_y_, join_unique_constraints));
  }
}
