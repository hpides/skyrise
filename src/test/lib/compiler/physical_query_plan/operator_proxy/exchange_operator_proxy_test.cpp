...
TEST(ExchangeOperatorProxyTest, Description) {
  ...
  EXPECT_EQ(exchange_proxy->Description(DescriptionMode::kSingleLine),
            "[Exchange] Combine Objects Target: 50 object(s), 1 partition(s)");
  EXPECT_EQ(exchange_proxy->Description(DescriptionMode::kMultiLine),
            "[Exchange]\nCombine Objects\nTarget: 50 object(s),\n1 partition(s)");
...
//     ImportOperatorProxy::Make(std::vector<ObjectReference>{ObjectReference("bucket_name", "import.orc")},
