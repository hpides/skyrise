...
class ExportOperatorProxyTest : public ::testing::Test {
 ...
 protected:
  static inline const ObjectReference kTargetObject{"export_bucket", "export.orc"};
  static inline const ObjectReference kImportObject{"import_bucket", "import.orc"};
...
TEST_F(ExportOperatorProxyTest, Description) {
  ...
  EXPECT_EQ(export_proxy->Description(DescriptionMode::kSingleLine), "[Export] export_bucket/export.orc");
...
TEST_F(ExportOperatorProxyTest, DeepCopy) {
  ...
  ExportOperatorProxy::Make(kTargetObject, ExportFormat::kCsv,
    ImportOperatorProxy::Make(std::vector<ObjectReference>{kImportObject}, std::vector<ColumnId>{ColumnId{0}}));
...
TEST_F(ExportOperatorProxyTest, CreateOperatorInstance) {
  ...
  ExportOperatorProxy::Make(kTargetObject, ExportFormat::kOrc,
    ImportOperatorProxy::Make(std::vector<ObjectReference>{kImportObject}, std::vector<ColumnId>{ColumnId{0}}));
