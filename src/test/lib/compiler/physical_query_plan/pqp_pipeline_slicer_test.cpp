...
class PqpPipelineSlicerTest : public ::testing::Test {
 public:
  ...
  std::shared_ptr<ExportOperatorProxy> CreateQ3Pqp(const std::vector<std::string>& import_keys_customer,
                                                   const std::vector<std::string>& import_keys_orders,
                                                   const std::vector<std::string>& import_keys_lineitem) {
    std::vector<ColumnID> column_ids = {ColumnID{4}, ColumnID{5}, ColumnID{6}, ColumnID{7},
                                        ColumnID{8}, ColumnID{9}, ColumnID{10}};
    // clang-format off
      ...
      ExchangeOperatorProxy::Make( // TODO partition
        FilterOperatorProxy::Make(GreaterThan_(PqpColumn_(ColumnId{0}, DataType::kLong, false, "l_shipdate"), "1995-03-18"),
          CreateTpchImportProxy(TpchTable::kLineItem, std::vector<std::string>{"l_orderkey", "l_extendedprice", "l_discount", "l_shipdate"}), CreateMockObjectReferences("lineitem_", 6000));
      ...
      ExchangeOperatorProxy // TODO partition
        FilterOperatorProxy::Make( // o_orderdate < 1995-03-18
          JoinOperatorProxy::Make( // o_custkey = c_custkey
            ExchangeOperatorProxy( // TODO partition
              CreateTpchImportProxy(TpchTable::kOrders, std::vector<std::string>{"o_orderkey", "o_custkey", "o_orderdate", "o_shippriority"}, CreateMockObjectReferences("orders_", 1500)),
            ExchangeOperatorProxy( // TODO partition
              FilterOperatorProxy::Make(Equals_(PqpColumn_(ColumnId{1}, DataType::kString, false, "c_mktsegment"), "AUTOMOBILE"),
                CreateTpchImportProxy(TpchTable::kCustomer, std::vector<std::string>{"c_custkey", "c_mktsegment"}, CreateMockObjectReferences("customer_", 150)))));
