...
TEST_F(PartitionOperatorProxyTest, BaseProperties) {
  const auto partiton_proxy = PartitionOperatorProxy::Make(partition_count_, partition_column_ids_);
...
TEST_F(PartitionOperatorProxyTest, SerializeAndDeserialize) {
  const auto proxy = PartitionOperatorProxy::Make(partition_count_, partition_column_ids_);
  // (1) Serialize
