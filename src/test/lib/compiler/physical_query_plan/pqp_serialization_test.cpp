...
TEST_F(PqpSerializationTest, SingleOperatorProxy) {
  ...
  auto import_proxy = ImportOperatorProxy::Make(bucket_name_, object_keys_, column_ids_);
  import_proxy->SetComment(comment);

  std::string serialized_proxy = SerializePqp(import_proxy);
  auto deserialized_proxy = DeserializePqp(serialized_proxy);
...
TEST_F(PqpSerializationTest, LinearOperatorChain) {
  auto import_proxy = ImportOperatorProxy::Make(bucket_name_, object_keys_, column_ids_);
  auto filter_proxy = FilterOperatorProxy::Make(
      GreaterThanEquals_(PqpColumn_(ColumnId{0}, DataType::kLong, false, "a"), 100), import_proxy);
  auto export_proxy = ExportOperatorProxy::Make(bucket_name_, target_key_, ExportFormat::kOrc, filter_proxy);
  ...
  auto deserialized_filter_proxy =
  ...
  auto deserialized_import_proxy =
...
TEST_F(PqpSerializationTest, OperatorTree) {
  ...
  auto filter_proxy2 = FilterOperatorProxy::Make(
      GreaterThanEquals_(PqpColumn_(ColumnId{0}, DataType::kLong, false, "a"), 100), import_proxy);
  auto union_all_proxy = UnionOperatorProxy::Make(SetOperationMode::kAll, filter_proxy1, filter_proxy2);

  std::string serialized_proxy = SerializePqp(union_all_proxy);
  auto deserialized_proxy = DeserializePqp(serialized_proxy);

  auto deserialized_union_all_proxy = std::dynamic_pointer_cast<UnionOperatorProxy>(deserialized_proxy);
  ASSERT_TRUE(deserialized_union_all_proxy);
  ASSERT_EQ(deserialized_union_all_proxy->InputNodeCount(), 2);

  auto deserialized_filter_proxy1 = deserialized_union_all_proxy->LeftInput();
  ASSERT_TRUE(deserialized_filter_proxy1);
  ASSERT_EQ(deserialized_filter_proxy1->InputNodeCount(), 1);

  auto deserialized_filter_proxy2 = deserialized_union_all_proxy->RightInput();
