...
TEST_F(AggregateOperatorProxyTest, BaseProperties) {
  auto aggregate_proxy = AggregateOperatorProxy::Make(group_by_column_ids_, aggregates_);
...
TEST_F(AggregateOperatorProxyTest, Description) {
  ...
  auto aggregates = ExpressionVector_(Sum_(c_));
...
TEST_F(AggregateOperatorProxyTest, SerializeAndDeserialize) {
  auto aggregate_proxy = AggregateOperatorProxy::Make(group_by_column_ids_, aggregates_);
  // (1) Serialize
  auto proxy_json = aggregate_proxy->ToJson();

  // (2) Deserialize & verify attributes
  auto deserialized_proxy = AggregateOperatorProxy::FromJson(proxy_json);
  auto deserialized_aggregate_proxy = std::dynamic_pointer_cast<AggregateOperatorProxy>(deserialized_proxy);
...
TEST_F(AggregateOperatorProxyTest, DeepCopy) {
  ...
  // clang-format on
  aggregate_proxy->SetIsPipelineBreaker(false);
  auto aggregate_proxy_copy = std::dynamic_pointer_cast<AggregateOperatorProxy>(aggregate_proxy->DeepCopy());
