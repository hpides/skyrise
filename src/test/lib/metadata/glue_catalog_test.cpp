#include "metadata/glue_catalog.hpp"
...
class AwsGlueCatalogTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    database_name_ = s3_bucket_directory_ + "_" + RandomString(10, kCharacterSetLower);
    catalog_ = std::make_shared<GlueCatalog>(client_, database_name_);
    EXPECT_EQ(catalog_->GetDatabaseName(), database_name_);

    s3_storage_ = std::make_shared<S3Storage>(client_->GetS3Client(), s3_bucket_);
  }
  ...
  ObjectReference AddRandomDataDirectory(size_t num_files) {
    ...
    return ObjectReference(s3_bucket_, database_name_);
  }
  ...
  const std::string s3_bucket_ = "skyrise-ci-bucket";
  const std::string s3_bucket_directory_ = "ci-results";
  std::shared_ptr<S3Storage> s3_storage_;
};

TEST_F(AwsGlueCatalogTest, CreateDeleteTableSchema) {
  const std::string table_name = RandomString(10);
...
TEST_F(AwsGlueCatalogTest, ListTableSchemas) {
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    expected_tables_names.push_back(RandomString(10, kCharacterSetLower));
