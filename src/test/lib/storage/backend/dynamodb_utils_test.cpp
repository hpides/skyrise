...
class AwsDynamoDbUtilsTest : public ::testing::Test {
  void SetUp() override {
    client_ = Client().GetDynamoDbClient();
    table_name_ = "skyrise-testing-" + RandomString(8);
    CreateDefaultTable();
  }
...
TEST_F(AwsDynamoDbUtilsTest, ActivateTimeToLive) {
  ...
  const auto ttl_specefication = outcome.GetResult().GetTimeToLiveSpecification();
  EXPECT_EQ(ttl_specefication.GetAttributeName(), ttl_attribute);
