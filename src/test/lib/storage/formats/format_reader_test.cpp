 ...
 * The test data is located in "resources/test_data/<format>" and must contain equivalent data for all readers.
 */
 ...
 protected:
  void SetUp() override {
    ...
    if (std::is_same_v<ParquetFormatReader, typename Formatter::first_type::first_type>) {
      ...
      file_format_ = "orc";
      invalid_file_error_message_ = "File size too small";
    }
  };
  ...
  std::unique_ptr<ObjectReader> GetObject() { return storage_.OpenForReading(object_name_); }
...
TYPED_TEST(FormatReaderTest, TypeSupport) {
  ...
  // ORC's timestamp granularity is seconds, whereas Parquet's granularity is ms/ns.
  // Therefore, we have to check for both granularities.
  EXPECT_TRUE(std::get<int64_t>((*chunk->GetSegment(0))[0]) == 1619535093 ||
              std::get<int64_t>((*chunk->GetSegment(0))[0]) == 1619535093000000);

  EXPECT_TRUE(std::get<int64_t>((*chunk->GetSegment(0))[1]) == 1619535093 ||
              std::get<int64_t>((*chunk->GetSegment(0))[1]) == 1619535093000000);
...
TYPED_TEST(FormatReaderTest, ReadInvalidFile) {
  ...
  auto writer = this->storage_.OpenForWriting(filename);
