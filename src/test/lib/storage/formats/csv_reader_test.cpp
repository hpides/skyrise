...
class CsvReaderTest : public ::testing::Test {
  void SetUp() override {
    ...
    testdata_storage_ = std::make_shared<TestStorage>();
  }
 ...
 protected:
  ...
  std::shared_ptr<TestStorage> testdata_storage_;
};
...
TEST_F(CsvReaderTest, LineItemExpectedChunks) {
  ...
  auto object_storage = std::make_shared<TestStorage>()->OpenForReading(kLineItemTblPath);
  ...
  std::cout << csv_reader->GetError().GetMessage() << "\n";
  ASSERT_FALSE(csv_reader->GetError());
}
