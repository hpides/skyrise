...
class MockChunkReader : public AbstractChunkReader {
 public:
  ...
  // The source parameter gets discarded and is only present to have an compatible interface.
  explicit MockChunkReader(std::unique_ptr<ObjectReader> source, Configuration configuration = Configuration());
