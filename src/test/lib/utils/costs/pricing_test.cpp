...
TEST_F(PricingTest, PricingLambda) {
  const std::function<void()> func = []() {
    ...
    const auto lambda_pricing1 = pricing.get_lambda_pricing();
    ...
    const auto lambda_pricing2 = pricing.get_lambda_pricing();
...
TEST_F(PricingTest, PricingS3) {
  const std::function<void()> func = []() {
    ...
    const auto s3_pricing1 = pricing.get_s3_pricing();
    ...
    const auto s3_pricing2 = pricing.get_s3_pricing();
