...
class FunctionHostInformationTest : public ::testing::Test {
 protected:
  ...
  FunctionHostInformationCollectorConfiguration config_{.cgroup_path = "/tmp/exampleCgroupFile.txt",
                                                        .ip_public_command = "echo \"5.6.7.8\"",
                                                        .collect_ip_public = false,
                                                        .operating_system_details_command = "echo \"OSDetails\"",
                                                        .ls_command = "ls",
                                                        .tmp_path = "tmp",
                                                        .stat_path = "/tmp/exampleStatFile.txt",
                                                        .uptime_path = "/tmp/exampleUptimeFile",
                                                        .cpuinfo_path = "/tmp/exampleCpuinfoFile",
                                                        .meminfo_path = "/tmp/exampleMeminfoFile",
                                                        .self_status_path = "/tmp/exampleSelfstatusFile",
                                                        .readable_json = true};
};
...
TEST_F(FunctionHostInformationTest, FunctionHostInformationTestJson) {
  ...
  const FunctionHostInformationIdentification mocked_information_identification{
      .id = information_identification.id, .ip_private = "1.2.3.4", .ip_public = information_identification.ip_public};
