...
class ImportOperatorTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    test_data_schema_ = std::make_shared<TableColumnDefinitions>();
    test_data_schema_->emplace_back("a_int", DataType::kInt, false);
    test_data_schema_->emplace_back("a_long", DataType::kLong, false);
    test_data_schema_->emplace_back("a_float", DataType::kFloat, false);
    test_data_schema_->emplace_back("a_double", DataType::kDouble, false);
    test_data_schema_->emplace_back("a_string", DataType::kString, false);
 ...
 protected:
  ...
  static constexpr std::string_view kParquetTypesPath = "parquet/with_types.parquet";

  CsvFormatReaderOptions csv_options_;
  OrcFormatReaderOptions orc_options_;
  ParquetFormatReaderOptions parquet_options_;

  std::shared_ptr<TableColumnDefinitions> test_data_schema_;
...
TEST_F(ImportOperatorTest, ImportCsv) {
  ...
  TestImportOperator(&import_operator, *test_data_schema_, 1, 1, &included_column_ids, operator_execution_context_);
}
...
TEST_F(ImportOperatorTest, ImportParquet) {
  ...
  const std::vector<ObjectReference> object_references = {ObjectReference{"TestStorage", kParquetTypesPath.data(), ""}};
  std::vector<ColumnId> included_column_ids = {ColumnId(0), ColumnId(1), ColumnId(2), ColumnId(3), ColumnId(4)};
