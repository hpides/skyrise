...
class FilterOperatorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    std::shared_ptr<Table> table = LoadTable<CsvFormatReader>("csv/input_a.csv", std::make_shared<TestdataStorage>());
  ...
  static void AssertColumnsAreEqual(const std::shared_ptr<const Table>& table, const ColumnId& column_id,
    ...
    for (ChunkId chunk_id = 0; chunk_id < table->ChunkCount(); ++chunk_id) {
      ...
      for (ChunkOffset chunk_offset = 0; chunk_offset < chunk->Size(); ++chunk_offset) {
        ...
        const auto search_iterator = std::find_if(expected.begin(), expected.end(), comparator);
