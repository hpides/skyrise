...
std::shared_ptr<const Table> CreateTableContainingValue(size_t num_chunks, ChunkOffset num_rows_per_chunk,
  ...
  MockChunkReader reader(nullptr, 0, config);
...
bool VerifyWrittenData(const FileFormat export_format, ByteBuffer* written_data, const size_t num_chunks,
                       const ChunkOffset num_rows_per_chunk) {
