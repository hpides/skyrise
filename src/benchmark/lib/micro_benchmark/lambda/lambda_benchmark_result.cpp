...
bool LambdaBenchmarkRepetitionResult::IsComplete() const {
  return std::ranges::all_of(invocation_results_,
...
long double LambdaBenchmarkRepetitionResult::CalculateRuntimeCost(
  ...
  std::ranges::transform(lambda_invocation_results, std::back_inserter(lambda_invocation_costs),
...
bool LambdaBenchmarkResult::IsResultComplete() const {
  return std::ranges::all_of(repetition_results_, [](const LambdaBenchmarkRepetitionResult& benchmark_repetition) {
    return benchmark_repetition.IsComplete();
  });
}
...
long double LambdaBenchmarkResult::CalculateRuntimeCost(const size_t function_instance_size_mb,
  ...
  std::ranges::transform(repetition_results, std::back_inserter(repetition_costs),
