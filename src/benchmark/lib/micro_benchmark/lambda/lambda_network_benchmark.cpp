...
const Aws::String kAmazonLinux2ImageIdArm = "ami-04b395c05193adbbd";
  ...
  for (const auto& function_instance_size_mb : function_instance_sizes_mb) {
    for (const auto& concurrent_instance_count : concurrent_instance_counts) {
      ...
      configs_.emplace_back(
          LambdaBenchmarkParameters{function_instance_size_mb, concurrent_instance_count, repetition_count},
          benchmark_config);
    }
  }
}
...
void LambdaNetworkBenchmark::Setup(const Aws::EC2::Model::InstanceType ec2_instance_type, const size_t ec2_cluster_size,
  ...
  for (size_t i = 0; i < ec2_cluster_size; ++i) {
    ec2_startup_futures.emplace_back(std::async([=]() {
      ...
      auto instance_request =
          Aws::EC2::Model::RunInstancesRequest()
              .WithImageId(kAmazonLinux2ImageIdArm)
...
std::string LambdaNetworkBenchmark::GenerateUserDataScript(const size_t start_port, const size_t server_count,
  ...
  Aws::String user_data_script = "#!/bin/bash\nyum install -y iperf3 && for i in {" + default_start_port + ".." +
                                 default_end_port +
  ...
  for (size_t i = 0; i < function_instance_mb_sizes.size(); ++i) {
    for (const auto& concurrent_invocation_count : concurrent_invocation_counts) {
      if (concurrent_invocation_count > kMaximumFunctionsPerInstance) {
        ...
        ec2_instance_types.push_back(Aws::EC2::Model::InstanceType::c6gn_16xlarge);
      } else {
        ...
        switch (concurrent_invocation_count) {
          ...
          case 3:
          case 4:
            ec2_instance_types.push_back(Aws::EC2::Model::InstanceType::c6gn_8xlarge);
            break;
          case 5:
          case 6:
            ec2_instance_types.push_back(Aws::EC2::Model::InstanceType::c6gn_12xlarge);
            break;
          case 7:
          case 8:
          case 9:
          case 10:
            ...
            break;
          default:
            Fail("A single EC2 instance can serve a maximum of 16 concurrent Lambda functions.");
        }
      }
    }
  }
