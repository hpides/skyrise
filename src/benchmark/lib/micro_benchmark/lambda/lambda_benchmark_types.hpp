...
enum class DeploymentType : std::uint8_t { kS3Object, kLocalZipFile };

enum class EventQueue : std::uint8_t { kNo, kYes };

enum class DistinctFunctionPerRepetition : std::uint8_t { kNo, kYes };

enum class Warmup : std::uint8_t { kNo, kYesOnInitialRepetition, kYesOnEveryRepetition };
