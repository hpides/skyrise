...
void LambdaStartupLatencyBenchmark::Setup() {
  ...
  if (std::find(deployment_types_.begin(), deployment_types_.end(), DeploymentType::kS3Object) !=
      deployment_types_.end()) {
  ...
  if (std::find(deployment_types_.begin(), deployment_types_.end(), DeploymentType::kLocalZipFile) !=
      deployment_types_.end()) {
    ...
    for (const auto function_instance_size_mb : function_instance_sizes_mb_) {
      for (const auto concurrent_instance_count : concurrent_instance_counts_) {
        for (const auto is_warm : warmup_options) {
          ...
          configs_.emplace_back(LambdaStartupBenchmarkParameters{{function_instance_size_mb, concurrent_instance_count,
                                                                  repetition_count_, 0},
                                                                 package_name,
                                                                 0,
                                                                 EventQueue::kNo,
