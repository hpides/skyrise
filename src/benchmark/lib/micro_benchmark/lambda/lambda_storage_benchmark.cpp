...
Aws::Utils::Json::JsonValue LambdaStorageBenchmark::GenerateResultOutput(
  ...
  return AddParameters(output, parameters)
      ...
      .WithDoubleRepetitionMetric([&](const LambdaBenchmarkRepetitionResult& repetition_result) {
        ...
        const double repetition_measurement_start_ms = *std::ranges::min_element(invocation_measurement_starts_ms);
        const double repetition_measurement_end_ms = *std::ranges::max_element(invocation_measurement_ends_ms);
