...
BenchmarkRunner::BenchmarkRunner(std::shared_ptr<const Aws::IAM::IAMClient> iam_client,
...
void BenchmarkRunner::WarmUpFunctions(const size_t repetition) {
  ...
  long double function_warm_up_cost = config_->warm_up_strategy_->WarmUpFunctions(
      lambda_client_, cost_calculator_, config_->function_configs_[function_index],
