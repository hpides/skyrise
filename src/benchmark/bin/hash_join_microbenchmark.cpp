...
class HashJoinMicrobenchmarkFixture : public benchmark::Fixture {
 public:
  void SetUp(benchmark::State& state) override {
    const auto table_left = CreateTableWithRandomizedInts(state.range(0) / 4, "a");
    const auto table_right = CreateTableWithRandomizedInts(state.range(0), "b");
  ...
  static std::shared_ptr<Table> CreateTableWithRandomizedInts(const size_t row_count, const std::string& column_name) {
