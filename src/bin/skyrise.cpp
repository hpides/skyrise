  ...
  try {
    ...
    if (parse_result.arguments().empty()) {
      ...
      Aws::InitAPI(options);
      {
        ...
        std::cout << outcome.GetResult().GetUser().GetUserName() << "\n";
      }
      ...
      if (parse_result.contains(kHelpOption)) {
    ...
    std::cerr << "Found no matching enum for name '" + parse_result[kToolOption].as<std::string>() + "'" << "\n";
    return 1;
