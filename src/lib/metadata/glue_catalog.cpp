...
// TODO(anyone): How do we handle types such as set or timestamp?
...
bool GlueCatalog::TableExists(const std::string& table_name) const {
  ...
  return cache_.find(lc_table_name) != cache_.end() ||
...
std::shared_ptr<GlueCatalog::TableMetadata> GlueCatalog::ToTableMetadata(const Aws::Glue::Model::Table& glue_table) {
  ...
  return std::make_shared<TableMetadata>(
      TableMetadata{glue_table.GetName(), glue_table.GetVersionId(), table_schema, location, manifest});
}
