...
ToolType ToolOptionToEnum(const std::string& tool_option) {
  // TODO(TheoRadig): Find a better way.
  std::smatch match;
  ...
  const std::string tool = match[1]
                               .str()
                               .replace(0, 1, boost::to_upper_copy(match[1].str().substr(0, 1)))
                               .insert(0, std::string(kConstPrefix));
...
cxxopts::Options ConfigureCliOptions() {
  ...
  cli_options.add_options()("h, help", kHelpHint);
