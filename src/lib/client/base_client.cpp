...
BaseClient::BaseClient() {
  ...
  auto dynamodb_client_configuration = GenerateClientConfig();
  dynamodb_client_configuration.retryStrategy = std::make_shared<CustomRetryStrategy>();

  const auto s3_endpoint_provider = std::make_shared<Aws::S3::S3EndpointProvider>();
  auto s3_client_configuration = GenerateClientConfig();
  s3_client_configuration.executor = std::make_shared<Aws::Utils::Threading::PooledThreadExecutor>(
  ...
  const std::vector<std::function<void()>> initializers{
      [&]() {
        dynamodb_client_ =
            std::make_shared<const Aws::DynamoDB::DynamoDBClient>(credentials_provider, dynamodb_client_configuration);
      },
      ...
      [&]() {
        s3_client_ = std::make_shared<const Aws::S3::S3Client>(credentials_provider, s3_endpoint_provider,
                                                               s3_client_configuration);
      },
