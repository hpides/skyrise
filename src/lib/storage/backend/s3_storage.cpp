...
skyrise::StorageError GetErrorFromOutcome(const AwsOutcomeClass& outcome) {
  const auto& error = outcome.GetError();
  return skyrise::StorageError(skyrise::TranslateS3Error(error.GetErrorType()), error.GetMessage());
}
...
Aws::S3::Model::GetObjectRequest S3ObjectReader::CreateGetObjectRequest(ByteBuffer* buffer, const std::string& range) {
  ...
  for (const auto& header : headers) {
    AWS_LOGSTREAM_DEBUG("S3ObjectReader", "Header: " << header.first << " = " << header.second);
  }
  return request;
}
