...
void CreateS3BucketIfNotExists(const std::shared_ptr<const Aws::S3::S3Client>& client, const Aws::String& bucket_name) {
  ...
  const auto contains_bucket_iterator =
      std::ranges::find_if(buckets, [&](const auto& bucket) { return bucket.GetName() == bucket_name; });
...
void EmptyS3Bucket(const std::shared_ptr<const Aws::S3::S3Client>& client, const Aws::String& bucket_name) {
  while (true) {
    ...
    std::ranges::transform(listed_objects, std::back_inserter(objects_to_delete), [](const auto& object) {
      return Aws::S3::Model::ObjectIdentifier().WithKey(object.GetKey());
    });
