...
class ParquetOutputProxy : public arrow::io::OutputStream {
 public:
  ...
  static std::shared_ptr<ParquetOutputProxy> Make(WriteCallback callback) {
    ...
    proxy->callback_ = callback;
    return proxy;
  }

  arrow::Status Close() { Fail("Close is not implemented for ParquetOutputProxy"); }

  arrow::Result<int64_t> Tell() const { return arrow::Result<int64_t>(bytes_written_); }
