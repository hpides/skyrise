...
bool CsvFormatReader::GuessHasTypeInformation(const Columns& columns) {
  ...
  return std::ranges::all_of(columns, [](const Lines& lines) {
    if (lines.size() < 2) {
      return false;
    }
...
char CsvFormatReader::GuessDelimiter(const Lines& lines) {
  ...
  const size_t maximum_index =
      std::ranges::distance(delimiter_probability.cbegin(), std::ranges::max_element(delimiter_probability));
...
StorageError CsvFormatReader::FillBuffer() {
  ...
  if (buffer_.size() == configuration_.read_buffer_size) {
    auto newline_offset = std::ranges::find(std::ranges::reverse_view(buffer_), '\n');
