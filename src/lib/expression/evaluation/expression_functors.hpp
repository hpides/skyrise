 ...
 */
template <typename T>
constexpr bool IsLogicalOperand = std::is_same_v<int32_t, T> || std::is_same_v<NullValue, T>;
...
struct TernaryOrEvaluator {
  ...
  struct Supports {
    static constexpr bool value = IsLogicalOperand<Result> && IsLogicalOperand<ArgA> && IsLogicalOperand<ArgB>;
  };
...
struct StlComparisonFunctorWrapper {
  ...
  template <typename Result, typename ArgA, typename ArgB>
  inline static constexpr bool supports_value = Supports<Result, ArgA, ArgB>::value;
