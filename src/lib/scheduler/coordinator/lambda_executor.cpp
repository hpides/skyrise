    ...
    lambda_client->InvokeAsync(
        ...
        [&](const Aws::Lambda::LambdaClient* /*client*/, const Aws::Lambda::Model::InvokeRequest& /*request*/,
          ...
          if (!outcome.IsSuccess()) {
            std::cerr << "Error invoking function: " << outcome.GetError().GetMessage() << std::endl;
          }
        });
    ...
    for (const auto& message : messages) {
      ...
      const auto fragment_result =
          std::make_shared<PqpPipelineFragmentExecutionResult>(PqpPipelineFragmentExecutionResult{
              pipeline_id, response_view.GetString(kWorkerResponseIdAttribute),
              response_view.GetInteger(kResponseIsSuccessAttribute) != 0,
              response_view.GetString(kResponseMessageAttribute),
              (size_t)response_view.GetInteger(kWorkerResponseRuntimeMsAttribute),
              (size_t)response_view.GetInt64(kWorkerResponseMemorySizeMbAttribute),
              (size_t)response_view.GetInteger(kWorkerResponseExportDataSizeBytesAttribute),
              response_view.GetObject(kResponseMeteringAttribute).Materialize()});
