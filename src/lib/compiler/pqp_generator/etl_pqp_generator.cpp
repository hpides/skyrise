...
std::vector<std::shared_ptr<PqpPipeline>> EtlPqpGenerator::GeneratePqp() const {
  ...
  for (size_t i = 0; i < object_count; i++) {
    ...
    output_objects.emplace_back(shuffle_storage_.bucket_name,
                                shuffle_storage_.identifier + "/" + std::to_string(i) + ".csv");
  }
...
std::shared_ptr<AbstractCompiler> EtlPqpGeneratorConfig::GenerateCompiler() const {
  return std::make_shared<EtlPqpGenerator>(query_id_, scale_factor_, shuffle_storage_);
}
