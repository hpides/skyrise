...
ExchangeResult::ExchangeResult(std::vector<PipelineFragmentDefinition> init_pipeline_fragment_definitions,
...
std::vector<ObjectReference> ExchangeResult::ObjectReferences() const {
  Assert(pipeline_fragment_definitions && !pipeline_fragment_definitions.empty(),
         "Missing PipelineFragmentDefinitions.");
...
AbstractExchangeStrategy::AbstractExchangeStrategy(const ExchangeType type) : type_(type) {}

ExchangeType AbstractExchangeStrategy::Type() const { return type_; }
