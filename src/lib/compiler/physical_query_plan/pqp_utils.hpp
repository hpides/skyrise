...
enum class PqpVisitation { kVisitInputs, kDoNotVisitInputs };
...
enum class PqpUpwardVisitation { kVisitOutputs, kDoNotVisitOutputs };
...
void VisitPqpUpwards(const std::shared_ptr<AbstractOperatorProxy>& pqp, Visitor visitor) {
  ...
  while (!queue.empty()) {
    ...
    if (!visited_operator_proxies.insert(operator_proxy).second) continue;
