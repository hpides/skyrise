...
bool UnionOperatorProxy::IsPipelineBreaker() const {
  // TODO(anyone): True, if the UNION operation is part of the original SQL query.
...
size_t UnionOperatorProxy::OutputObjectsCount() const {
  // TODO(anyone): The function result depends on the Union operator implementation, which is currently missing.
