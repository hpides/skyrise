...
void AbstractAggregateOperator::ValidateAggregates() const {
  ...
  for (const auto& aggregate : aggregates_) {
    ...
    const ColumnId column_id = pqp_column->column_id_;
    if (column_id == kInvalidColumnId) {
      ...
      Assert(input_table->ColumnDataType(column_id) != DataType::kString ||
                 (aggregate->aggregate_function_ != AggregateFunction::kSum &&
                  aggregate->aggregate_function_ != AggregateFunction::kAvg &&
                  aggregate->aggregate_function_ != AggregateFunction::kStandardDeviationSample),
