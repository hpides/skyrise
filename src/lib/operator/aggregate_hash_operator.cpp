...
constexpr uint64_t kCacheMask = skyrise::AggregateKeyEntry{1} << 63u;
...
typename Results::reference GetOrAddResult(CacheResultIds /*cache_result_ids*/, ResultIds& result_ids, Results& results,
                                           AggregateKey& key, const skyrise::RowId& row_id) {
  if constexpr (std::is_same_v<AggregateKey, skyrise::EmptyAggregateKey>) {
    ...
    skyrise::AggregateKeyEntry* first_key_entry = nullptr;
    if constexpr (std::is_same_v<AggregateKey, skyrise::AggregateKeyEntry>) {
      first_key_entry = &key;
    } else {
      first_key_entry = &key[0];
    }

    static_assert(std::is_same_v<skyrise::AggregateKeyEntry, uint64_t>,
    ...
    if constexpr (std::is_same_v<CacheResultIds, std::true_type>) {
      if (*first_key_entry & kCacheMask) {
        ...
        const skyrise::AggregateKeyEntry result_id = *first_key_entry ^ kCacheMask;
...
AggregateKey& GetAggregateKey([[maybe_unused]] skyrise::KeysPerChunk<AggregateKey>& keys_per_chunk,
                              [[maybe_unused]] const skyrise::ChunkId chunk_id,
                              [[maybe_unused]] const skyrise::ChunkOffset chunk_offset) {
  if constexpr (!std::is_same_v<AggregateKey, skyrise::EmptyAggregateKey>) {
    ...
    // every call.
    static skyrise::EmptyAggregateKey empty_aggregate_key;
 ...
 */
template <typename ColumnDataType, typename AggregateType, skyrise::AggregateFunction AggregateFunc>
std::enable_if_t<
    AggregateFunc == skyrise::AggregateFunction::kMin || AggregateFunc == skyrise::AggregateFunction::kMax ||
        AggregateFunc == skyrise::AggregateFunction::kSum || AggregateFunc == skyrise::AggregateFunction::kAny,
    void>
WriteAggregateValues(std::vector<AggregateType>& values, std::vector<bool>& null_values,
                     const skyrise::AggregateResults<ColumnDataType, AggregateFunc>& results,
                     const std::shared_ptr<skyrise::AggregateExpression>& /*aggregate_function*/,
                     const std::shared_ptr<const skyrise::Table>& /*input_table*/) {
 ...
 */
template <typename ColumnDataType, typename AggregateType, skyrise::AggregateFunction AggregateFunc>
std::enable_if_t<AggregateFunc == skyrise::AggregateFunction::kCount, void> WriteAggregateValues(
 ...
 */
template <typename ColumnDataType, typename AggregateType, skyrise::AggregateFunction AggregateFunc>
std::enable_if_t<AggregateFunc == skyrise::AggregateFunction::kCountDistinct, void> WriteAggregateValues(
    std::vector<AggregateType>& values, std::vector<bool>& /*null_values*/,
    const skyrise::AggregateResults<ColumnDataType, AggregateFunc>& results,
    const std::shared_ptr<skyrise::AggregateExpression>& /*aggregate_function*/,
    const std::shared_ptr<const skyrise::Table>& /*input_table*/) {
 ...
 */
template <typename ColumnDataType, typename AggregateType, skyrise::AggregateFunction AggregateFunc>
std::enable_if_t<AggregateFunc == skyrise::AggregateFunction::kAvg && std::is_arithmetic_v<AggregateType>, void>
WriteAggregateValues(std::vector<AggregateType>& values, std::vector<bool>& null_values,
                     const skyrise::AggregateResults<ColumnDataType, AggregateFunc>& results,
                     const std::shared_ptr<skyrise::AggregateExpression>& /*aggregate_function*/,
                     const std::shared_ptr<const skyrise::Table>& /*input_table*/) {
...
skyrise::RowIdPositionList& MaterializePositions(
    skyrise::RowIdPositionList& previously_sorted_position_list,
    std::vector<std::pair<skyrise::RowId, SortColumnDataType>>& row_id_value_pairs) {
  skyrise::RowIdPositionList position_list;
...
std::vector<std::pair<skyrise::RowId, SortColumnDataType>> MaterializeSortColumn(
    const std::shared_ptr<const skyrise::Table>& input_table,
    std::vector<std::pair<skyrise::RowId, AggregateType>>& result_values,
    skyrise::RowIdPositionList& previously_sorted_position_list, const skyrise::SortColumnDefinition& definition) {
  std::vector<std::pair<skyrise::RowId, SortColumnDataType>> row_id_value_pairs;
  if (!previously_sorted_position_list.empty()) {
    for (const auto& row_id : previously_sorted_position_list) {
      const skyrise::ChunkId chunk_id = row_id.chunk_id;
      const skyrise::ChunkOffset chunk_offset = row_id.chunk_offset;
      const auto value_segment = std::static_pointer_cast<skyrise::ValueSegment<SortColumnDataType>>(
          input_table->GetChunk(chunk_id)->GetSegment(definition.column_id));
      row_id_value_pairs.emplace_back(skyrise::RowId{.chunk_id = chunk_id, .chunk_offset = chunk_offset},
                                      value_segment->Get(chunk_offset));
    }
  } else {
    for (const auto& value : result_values) {
      const skyrise::ChunkId chunk_id = value.first.chunk_id;
      const skyrise::ChunkOffset chunk_offset = value.first.chunk_offset;
      const auto value_segment = std::static_pointer_cast<skyrise::ValueSegment<SortColumnDataType>>(
          input_table->GetChunk(chunk_id)->GetSegment(definition.column_id));
      row_id_value_pairs.emplace_back(skyrise::RowId{.chunk_id = chunk_id, .chunk_offset = chunk_offset},
                                      value_segment->Get(chunk_offset));
...
template <typename SortColumnDataType>
void Sort(const skyrise::SortColumnDefinition& definition,
          std::vector<std::pair<skyrise::RowId, SortColumnDataType>>& row_id_value_pairs) {
  const auto sort = [&](auto comparator) {
    std::stable_sort(
        row_id_value_pairs.begin(), row_id_value_pairs.end(),
        // NOLINTNEXTLINE(performance-unnecessary-value-param)
        [&comparator](std::pair<skyrise::RowId, SortColumnDataType> a,
                      // NOLINTNEXTLINE(performance-unnecessary-value-param)
                      std::pair<skyrise::RowId, SortColumnDataType> b) { return comparator(a.second, b.second); });
  };

  if (definition.sort_mode == skyrise::SortMode::kAscending) {
    sort(std::less<>());
  } else {
    sort(std::greater<>());
  }
...
template <typename ColumnDataType, typename AggregateType, skyrise::AggregateFunction AggregateFunc>
std::enable_if_t<AggregateFunc == skyrise::AggregateFunction::kStringAgg, void> WriteAggregateValues(
    std::vector<AggregateType>& values, std::vector<bool>& null_values,
    const skyrise::AggregateResults<ColumnDataType, AggregateFunc>& results,
    const std::shared_ptr<skyrise::AggregateExpression>& aggregate_function,
    const std::shared_ptr<const skyrise::Table>& input_table) {
  ...
  for (const auto& result : results) {
    ...
    std::vector<std::pair<skyrise::RowId, AggregateType>> result_values = std::move(result.accumulator);
    const auto string_agg_expression =
        std::static_pointer_cast<skyrise::StringAggAggregateExpression>(aggregate_function);
    if (string_agg_expression->OrderWithinGroup()) {
      // With prior sorting.
      const std::vector<skyrise::SortColumnDefinition> sort_column_definitions =
          string_agg_expression->SortColumnDefinitions();
      skyrise::RowIdPositionList previously_sorted_position_list;
      ...
      for (const auto& row_id : previously_sorted_position_list) {
        ...
        const skyrise::ColumnId string_agg_column_id =
            std::static_pointer_cast<skyrise::PqpColumnExpression>(aggregate_function->Argument())->GetColumnId();
        const auto value_segment = std::static_pointer_cast<skyrise::ValueSegment<std::string>>(
...
template <typename ColumnDataType, typename AggregateType, skyrise::AggregateFunction AggregateFunc>
std::enable_if_t<
    AggregateFunc == skyrise::AggregateFunction::kStandardDeviationSample && std::is_arithmetic_v<AggregateType>, void>
WriteAggregateValues(std::vector<AggregateType>& values, std::vector<bool>& null_values,
                     const skyrise::AggregateResults<ColumnDataType, AggregateFunc>& results,
                     const std::shared_ptr<skyrise::AggregateExpression>& /*aggregate_function*/,
                     const std::shared_ptr<const skyrise::Table>& /*input_table*/) {
...
AggregateHashOperator::AggregateHashOperator(std::shared_ptr<AbstractOperator> input_operator,
    ...
    : AbstractAggregateOperator(std::move(input_operator), std::move(aggregates), std::move(group_by_column_ids)),
      has_aggregate_functions_(!aggregates_.empty() &&
                               !std::ranges::all_of(aggregates_, [](const auto& aggregate_expression) {
...
__attribute__((hot)) void AggregateHashOperator::AggregateSegment(ChunkId chunk_id, ColumnId column_index,
  ...
  const auto process_position = [&](const auto cache_result_ids, const AllTypeVariant& value) {
    auto& result = GetOrAddResult(cache_result_ids, result_ids, results,
                                  ...
                                  RowId{.chunk_id = chunk_id, .chunk_offset = chunk_offset});
                  ...
                  if constexpr (std::is_same_v<ColumnDataType, std::string>) {
                    ...
                    if (string_value.size() < kStringLengthThreshold) {
                      ...
                      switch (string_value.size()) {
                        ...
                        default: {
                          // For strings with length >= 5, we use the id_map as mentioned in the comment above
                          // This case is handled outside the switch statement, so we do not need to do anything here.
                        } break;
                      }
                    }
                  }
      ...
      if (use_immediate_key_shortcut_) {
        for (ChunkOffset j = 0; j < input_chunk_size; ++j) {
          ...
          GetOrAddResult(std::true_type(), result_ids, results, GetAggregateKey<AggregateKey>(keys_per_chunk, i, j),
                         RowId{.chunk_id = i, .chunk_offset = j});
        }
      } else {
      ...
      for (const auto& aggregate : aggregates_) {
        ...
        if (input_column_id == kInvalidColumnId) {
          ...
          if constexpr (std::is_same_v<AggregateKey, EmptyAggregateKey>) {
            ...
            // not kNullRowId.
            results.front().row_id = RowId{.chunk_id = 0, .chunk_offset = 0};
          } else {
            ...
            if (contexts_per_column_.size() > 1 || use_immediate_key_shortcut_) {
              for (ChunkOffset j = 0; j < input_chunk_size; ++j) {
                auto& result = GetOrAddResult(std::true_type(), result_ids, results,
                                              GetAggregateKey<AggregateKey>(keys_per_chunk, i, j),
                                              RowId{.chunk_id = i, .chunk_offset = j});
                ++result.aggregate_count;
              }
            } else {
