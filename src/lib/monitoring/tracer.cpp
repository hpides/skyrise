...
Tracer::~Tracer() {
  subsegments_[current_operator_id_].emplace_back(
      StageInformation{.stage = "OperatorExit", .timestamp = std::chrono::system_clock::now()});
  SendTrace();
}
...
void Tracer::EnterStage(const std::string& stage) {
  subsegments_[current_operator_id_].emplace_back(
      StageInformation{.stage = stage, .timestamp = std::chrono::system_clock::now()});
}
...
void Tracer::SendTrace() {
  ...
  for (const auto& [operator_id, subsegment_information] : subsegments_) {
    ...
    const Subsegment operator_subsegment{
        .id = operator_subsegment_id,
        .name = operator_id,
        .start_time =
            std::chrono::duration<double>(subsegment_information.front().timestamp.time_since_epoch()).count(),
        .end_time = std::chrono::duration<double>(subsegment_information.back().timestamp.time_since_epoch()).count(),
        .trace_id = trace_id_,
    ...
    for (size_t i = 0; i < subsegment_information.size() - 1; ++i) {
      ...
      const Subsegment stage_subsegment{
          .id = stage_subsegment_id,
          .name = subsegment_information[i].stage,
          .start_time = std::chrono::duration<double>(subsegment_information[i].timestamp.time_since_epoch()).count(),
          .end_time = std::chrono::duration<double>(subsegment_information[i + 1].timestamp.time_since_epoch()).count(),
          .trace_id = trace_id_,
          .parent_id = operator_subsegment_id};
