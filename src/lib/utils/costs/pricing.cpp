...
Pricing::Pricing(std::shared_ptr<const Aws::Pricing::PricingClient> pricing_client, const std::string& client_region)
  ...
  if (cpu_information.machine == std::string("aarch64")) {
    pricing_lambda_ = std::make_shared<PricingLambda>(
        PricingLambda{pricing_lambda_map.at(usage_type_lambda::kRequestArm),
                      pricing_lambda_map.at(usage_type_lambda::kLambdaGBSecondArm),
                      pricing_lambda_map.at(usage_type_lambda::kLambdaProvisionedGBSecondArm),
                      pricing_lambda_map.at(usage_type_lambda::kLambdaProvisionedConcurrencyArm)});
  } else {
    pricing_lambda_ = std::make_shared<PricingLambda>(
        PricingLambda{pricing_lambda_map.at(usage_type_lambda::kRequestX86),
                      pricing_lambda_map.at(usage_type_lambda::kLambdaGBSecondX86),
                      pricing_lambda_map.at(usage_type_lambda::kLambdaProvisionedGBSecondX86),
                      pricing_lambda_map.at(usage_type_lambda::kLambdaProvisionedConcurrencyX86)});
  }

  const auto pricing_s3_map = FetchPricing("AmazonS3");
  pricing_s3_ = std::make_shared<PricingS3>(PricingS3{
      pricing_s3_map.at(usage_type_s3::kRequestTier1), pricing_s3_map.at(usage_type_s3::kRequestTier2),
      pricing_s3_map.at(usage_type_s3::kSelectReturnedBytes), pricing_s3_map.at(usage_type_s3::kSelectScannedBytes),
      pricing_s3_map.at(usage_type_s3::kTagStorage), pricing_s3_map.at(usage_type_s3::kTimedStorage)});

  const auto pricing_xray_map = FetchPricing("AWSXRay");
  pricing_xray_ = std::make_shared<PricingXray>(PricingXray{pricing_xray_map.at(usage_type_xray::kXrayTracesAccessed),
                                                            pricing_xray_map.at(usage_type_xray::kXrayTracesStored)});
}
