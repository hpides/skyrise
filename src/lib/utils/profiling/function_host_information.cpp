...
FunctionHostInformationIdentification FunctionHostInformationCollector::CollectInformationIdentification() {
  return {.id = Id(), .ip_private = IpPrivate(), .ip_public = IpPublic()};
}

FunctionHostInformationEnvironment FunctionHostInformationCollector::CollectInformationEnvironment() {
  return {.operating_system_details = OperatingSystemDetails(),
          .file_system_details = FileSystemDetails(),
...
FunctionHostInformationResources FunctionHostInformationCollector::CollectInformationResources() {
  ...
  return {.cpu_count = cpu_info.cpu_count,
          .cpu_model = cpu_info.cpu_model,
          .cpu_features = cpu_info.cpu_features,
          .ram_size_mb = RamSizeMb()};
}
...
FunctionHostInformationResourceUsage FunctionHostInformationCollector::CollectInformationResourceUsage() {
  ...
  return {.process_used_memory_in_ram_kb = process_memory.physical_memory_kb,
          .process_used_memory_kb = process_memory.virtual_memory_kb,
          .system_available_memory_kb = system_memory.available_memory_kb,
          .system_free_memory_kb = system_memory.free_memory_kb};
}
...
FunctionHostInformationCollector::CpuInfo FunctionHostInformationCollector::CpuInformation() const {
  ...
  return {.cpu_count = cpu_count, .cpu_model = cpu_model, .cpu_features = cpu_features};
}
...
SystemMemoryUsage FunctionHostInformationCollector::GetSystemMemoryUsage() const {
  ...
  return {.available_memory_kb = GetKbMemorySizeFromFile("MemAvailable", file_content),
          .free_memory_kb = GetKbMemorySizeFromFile("MemFree", file_content)};
}
...
ProcessMemoryUsage FunctionHostInformationCollector::GetProcessMemoryUsage() const {
  ...
  return {.physical_memory_kb = GetKbMemorySizeFromFile("VmRSS", file_content),
          .virtual_memory_kb = GetKbMemorySizeFromFile("VmSize", file_content)};
}
